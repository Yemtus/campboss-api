...
router.post('/submit', async (req, res, next) => {
  try {
    ...
    await sendEmail(email, 'Your Camp Boss Demo Access', `
        ...
        <div style="background:#1e293b;border:1px solid #334155;border-radius:10px;padding:1.5rem;margin-bottom:2rem;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
            <div style="background:#0f172a;border:1px solid #334155;border-radius:8px;padding:0.75rem;">
              <p style="font-size:0.7rem;color:#64748b;text-transform:uppercase;margin-bottom:0.25rem;">Username</p>
              <p style="font-family:monospace;color:#f97316;font-weight:700;font-size:1.1rem;">demo</p>
            </div>
            <div style="background:#0f172a;border:1px solid #334155;border-radius:8px;padding:0.75rem;">
              <p style="font-size:0.7rem;color:#64748b;text-transform:uppercase;margin-bottom:0.25rem;">Password</p>
              <p style="font-family:monospace;color:#f97316;font-weight:700;font-size:1.1rem;">demo123</p>
            </div>
          </div>
        </div>
        <p style="color:#94a3b8;margin-bottom:1rem;">Pre-loaded with realistic offshore catering data. Explore every feature straight away. Data resets automatically every midnight.</p>
    ...
    await sendEmail(process.env.NOTIFY_EMAIL, `New Camp Boss Lead - ${company}`, `
      <div style="font-family:Arial,sans-serif;max-width:500px;margin:0 auto;padding:2rem;">
        ...
        <table style="width:100%;border-collapse:collapse;">
          <tr><td style="padding:0.5rem 0;color:#64748b;">Name</td><td style="padding:0.5rem 0;font-weight:600;">${name}</td></tr>
          <tr><td style="padding:0.5rem 0;color:#64748b;">Company</td><td style="padding:0.5rem 0;font-weight:600;">${company}</td></tr>
          <tr><td style="padding:0.5rem 0;color:#64748b;">Email</td><td style="padding:0.5rem 0;font-weight:600;">${email}</td></tr>
          <tr><td style="padding:0.5rem 0;color:#64748b;">Phone</td><td style="padding:0.5rem 0;font-weight:600;">${phone || 'Not provided'}</td></tr>
          <tr><td style="padding:0.5rem 0;color:#64748b;">Camp Size</td><td style="padding:0.5rem 0;font-weight:600;">${camp_size || 'Not provided'}</td></tr>
        </table>
    ...
    console.error('[Leads]', err.response?.data || err.message);
    next(err);
  }
});
